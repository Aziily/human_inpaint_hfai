...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        if f"{args.output_dir}" is not None:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if dist.get_rank() == 0 and hfai.receive_suspend_command():
                    ...
                    # pipeline.save_pretrained(os.path.join(f"{args.output_dir}", './latest/'))
                    torch.save(text_encoder.module, os.path.join(os.path.join(f"{args.output_dir}", './latest/'), './text_encoder.model'))
                    torch.save(vae.module, os.path.join(os.path.join(f"{args.output_dir}", './latest/'), './vae.model'))
                    torch.save(unet.module, os.path.join(os.path.join(f"{args.output_dir}", './latest/'), './unet.model'))
                    time.sleep(5)
                    hfai.go_suspend()
    ...
    if dist.get_rank() == 0:
        ...
        #     text_encoder=text_encoder,
        # )
        # pipeline.save_pretrained(f"{args.output_dir}")
        if not os.path.exists(os.path.join(f"{args.output_dir}", './final/')): os.mkdir(os.path.join(f"{args.output_dir}", './final/'))
        # pipeline.save_pretrained(os.path.join(f"{args.output_dir}", './latest/'))
        torch.save(text_encoder.module, os.path.join(os.path.join(f"{args.output_dir}", './final/'), './text_encoder.model'))
        torch.save(vae.module, os.path.join(os.path.join(f"{args.output_dir}", './final/'), './vae.model'))
        torch.save(unet.module, os.path.join(os.path.join(f"{args.output_dir}", './final/'), './unet.model'))
